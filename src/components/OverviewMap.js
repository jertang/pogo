  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .overview-map {
        ...
        width: 100%;
        margin-top: 0; /* Remove margin since we'll position absolutely */
        position: absolute;
        top: 0;
        left: 0;
      }
      ...
      .state-info-panel {
        ...
        z-index: 1000; /* Lower z-index so navigation bar stays on top */
        overflow-y: auto;
      ...
      .state-info-header {
        ...
        color: white;
        position: relative;
        min-height: 80px; /* Ensure enough space for the close button */
      }
      ...
      .legend {
        position: absolute;
        bottom: 80px; /* Move up a bit to ensure visibility */
        left: 24px;
        background: white;
        padding: 20px;
        border-radius: 12px;
